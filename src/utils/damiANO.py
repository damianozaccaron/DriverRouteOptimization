...
def implement_pref(data, output_len):
    pref = Preferences(
        ...
        freq_merch_avg=mean_types(data),
        n_merch=sorted(count_merch(data).items(), key=lambda item: item[1], reverse=True)[0:output_len],
        n_merch_per_route=mean_quantities(data),
        freq_merch_per_trip=sorted(
            utils.frequent_itemset.run_pcy(extract_merchandise_type(data), n_buckets=600, t_hold=0.2,

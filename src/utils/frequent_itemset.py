...
def hash_pair(n1, n2, n_buckets):
    """Generates a basic hash function starting from string"""
...
def pcy_basket(basket, n_buckets, pairs_hashtable, singletons):
...
def run_pcy(baskets, n_buckets, t_hold, start=time.time()):
    ...
    for item in baskets:
        pcy_basket(item, n_buckets, pairs_count_hash, singletons)
    ...
    "now we have all the candidate pairs, we want to count how much they appear together in the same route"
    frequent_pairs = {}
...
luciano = run_pcy(merch.extract_merch_city(data), n_buckets=num_buckets, t_hold=0.1, start=time.time())
print(luciano)

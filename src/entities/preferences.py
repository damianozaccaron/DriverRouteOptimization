...
pref = Preferences(
    freq_city=sorted(trips.pass_through_city_count(data).items(), key=lambda item: item[1], reverse=True)[0:output_len],
    freq_start=sorted(trips.start_finish_count(data, 0).items(), key=lambda item: item[1], reverse=True)[0:output_len],
    freq_finish=sorted(trips.start_finish_count(data).items(), key=lambda item: item[1], reverse=True)[0:output_len],
    ...
    freq_merch_per_trip=sorted(frequent_itemset.run_pcy(merch.extract_merchandise_type(data), n_buckets=600, t_hold=0.2,
                                    start=time.time()).items(), key=lambda item: item[1], reverse=True)[0:output_len]
)
